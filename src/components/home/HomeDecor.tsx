...
interface HomeDecorProps {
  fineArtsPrints?: Record<string, string>; // Object with key-value pairs (strings)
}

const HomeDecor: FC<HomeDecorProps> = ({ fineArtsPrints }) => {
  if (!fineArtsPrints || Object.keys(fineArtsPrints).length === 0) return null;
  ...
  return (
    <div className="font-poppins bg-black text-[#F8C900] p-6 w-full flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-12">
      ...
      <div className="flex flex-wrap justify-start w-full md:w-auto gap-4 md:gap-x-16">
        {Object.entries(fineArtsPrints).map(([category, value]) => (
